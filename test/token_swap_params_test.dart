import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for TokenSwapParams
void main() {
  final instance = TokenSwapParamsBuilder();
  // TODO add properties to the builder and call build()

  group(TokenSwapParams, () {
    // String to
    test('to test the property `to`', () async {
      // TODO
    });

    // String data
    test('to test the property `data`', () async {
      // TODO
    });

    // String input
    test('to test the property `input`', () async {
      // TODO
    });

    // String value
    test('to test the property `value`', () async {
      // TODO
    });

    // String nonce
    test('to test the property `nonce`', () async {
      // TODO
    });

    // String gas
    test('to test the property `gas`', () async {
      // TODO
    });

    // String gasPrice
    test('to test the property `gasPrice`', () async {
      // TODO
    });

    // String chainId
    test('to test the property `chainId`', () async {
      // TODO
    });

    // String encoding
    test('to test the property `encoding`', () async {
      // TODO
    });

    // bool EOA
    test('to test the property `EOA`', () async {
      // TODO
    });

    // String contractAddress
    test('to test the property `contractAddress`', () async {
      // TODO
    });

    // String tokenId
    test('to test the property `tokenId`', () async {
      // TODO
    });

    // String tokenIds
    test('to test the property `tokenIds`', () async {
      // TODO
    });

    // bool approved
    test('to test the property `approved`', () async {
      // TODO
    });

    // bool broadcast
    test('to test the property `broadcast`', () async {
      // TODO
    });

    // String tokenIn
    test('to test the property `tokenIn`', () async {
      // TODO
    });

    // String tokenOut
    test('to test the property `tokenOut`', () async {
      // TODO
    });

    // double tokenInDecimals
    test('to test the property `tokenInDecimals`', () async {
      // TODO
    });

    // double tokenOutDecimals
    test('to test the property `tokenOutDecimals`', () async {
      // TODO
    });

    // String amountIn
    test('to test the property `amountIn`', () async {
      // TODO
    });

    // String slippage
    test('to test the property `slippage`', () async {
      // TODO
    });

    // String recipient
    test('to test the property `recipient`', () async {
      // TODO
    });

    // String referrer
    test('to test the property `referrer`', () async {
      // TODO
    });

  });
}
