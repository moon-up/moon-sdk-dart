import 'package:test/test.dart';
import 'package:moonsdk/moonsdk.dart';

// tests for AaveInput
void main() {
  final instance = AaveInputBuilder();
  // TODO add properties to the builder and call build()

  group(AaveInput, () {
    // String to
    test('to test the property `to`', () async {
      // TODO
    });

    // String data
    test('to test the property `data`', () async {
      // TODO
    });

    // String input
    test('to test the property `input`', () async {
      // TODO
    });

    // String value
    test('to test the property `value`', () async {
      // TODO
    });

    // String nonce
    test('to test the property `nonce`', () async {
      // TODO
    });

    // String gas
    test('to test the property `gas`', () async {
      // TODO
    });

    // String gasPrice
    test('to test the property `gasPrice`', () async {
      // TODO
    });

    // String chainId
    test('to test the property `chainId`', () async {
      // TODO
    });

    // String encoding
    test('to test the property `encoding`', () async {
      // TODO
    });

    // bool EOA
    test('to test the property `EOA`', () async {
      // TODO
    });

    // String contractAddress
    test('to test the property `contractAddress`', () async {
      // TODO
    });

    // String tokenId
    test('to test the property `tokenId`', () async {
      // TODO
    });

    // String tokenIds
    test('to test the property `tokenIds`', () async {
      // TODO
    });

    // bool approved
    test('to test the property `approved`', () async {
      // TODO
    });

    // bool broadcast
    test('to test the property `broadcast`', () async {
      // TODO
    });

    // String lendingPool
    test('to test the property `lendingPool`', () async {
      // TODO
    });

    // String amount
    test('to test the property `amount`', () async {
      // TODO
    });

    // String atokenToRedeeem
    test('to test the property `atokenToRedeeem`', () async {
      // TODO
    });

    // String refCode
    test('to test the property `refCode`', () async {
      // TODO
    });

    // String interestRateMode
    test('to test the property `interestRateMode`', () async {
      // TODO
    });

  });
}
